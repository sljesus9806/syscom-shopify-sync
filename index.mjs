...
// SHOP (subdominio sin .myshopify.com), ADMIN_TOKEN (shpat_...)
// SYSCOM_CLIENT_ID, SYSCOM_CLIENT_SECRET
// SYSCOM_MODE: "search" | "brand"
// SYSCOM_QUERY: si MODE=search -> término (ej. "camaras") / si MODE=brand -> slug (ej. "hikvision")
// RUN_PAGES: páginas por corrida (ej. "2")
// SLEEP_MS: milisegundos entre productos (ej. "900")
...
const MODE = process.env.SYSCOM_MODE || "search";
...
async function getPublicationId(){
  ...
  return d.publications.edges[0].node.id; // primera (Online Store usualmente)
}
...
async function findVariantBySku(sku){
  const q = `query ($q:String!){ productVariants(first:1, query:$q){ edges{ node{ id sku product{ id status } inventoryItem{ id } } } } }`;
...
async function productCreate({title, descriptionHtml, vendor, productType, images}){
  const q = `mutation CreateProduct($product: ProductCreateInput!, $media: [CreateMediaInput!]){
    productCreate(product:$product, media:$media){
      product{ id variants(first:1){ nodes{ id inventoryItem{ id } } } }
      userErrors{ field message }
    }}`;
  const media = (images||[]).filter(Boolean).map(u => ({originalSource:u})).slice(0,10);
  ...
  return { productId: p.id, variantId: p.variants.nodes[0].id, inventoryItemId: p.variants.nodes[0].inventoryItem.id };
}
async function updateVariantPrice(productId, variantId, price){
  const q = `mutation UpdateVariants($productId:ID!, $variants:[ProductVariantsBulkInput!]!){
    productVariantsBulkUpdate(productId:$productId, variants:$variants){ userErrors{ field message } }}`;
  const d = await gql(q, {productId, variants:[{id: variantId, price: String(price)}]});
  const e = d.productVariantsBulkUpdate.userErrors;
  if(e?.length) throw new Error(JSON.stringify(e));
}
async function setInventorySku(inventoryItemId, sku){
  const q = `mutation InvItemUpdate($id:ID!, $input:InventoryItemInput!){
...
async function adjustInventory(inventoryItemId, location, targetQty){
  ...
  if(!delta) return;
  const q = `mutation Adjust($input:InventoryAdjustQuantitiesInput!){
...
async function publishProduct(productId, publicationId){
  const q = `mutation Pub($id:ID!, $input:[PublicationInput!]!){
...
function mapSyscomProduct(P){
  ...
  return { sku:String(sku), title:String(title), descriptionHtml:String(desc), vendor:String(vendor),
           productType:String(ptype), price, available:Number(qty), images, barcode };
}
